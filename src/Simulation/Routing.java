...
public class Routing {
    ...
    public Routing(main_frame parent, AllData allData, int traffic, int Passed_lavaIndex) {
        myParent = parent;
        myAllData = allData;
    ...
    private double distance(LocationNode start, LocationNode end) {
        ...
        for (int i = 0; i < startIndexes.length; i++) {
            WV = ((LayerDefinition) myParent.allData.all_Layers.get(0)).values[startIndexes[i] - 1];
        }
        for (int i = 0; i < endIndexes.length; i++) {
            WV = ((LayerDefinition) myParent.allData.all_Layers.get(0)).values[endIndexes[i] - 1];
        }
        ...
        if (trafficLayer > -1) {
            ...
            if (start.layers.get(trafficLayer) instanceof short[]) {
                ...
                double startTraffic = ((LayerDefinition) myParent.allData.all_Layers.get(0)).values[startIndex - 1];
                double endTraffic = ((LayerDefinition) myParent.allData.all_Layers.get(0)).values[endIndex - 1];
