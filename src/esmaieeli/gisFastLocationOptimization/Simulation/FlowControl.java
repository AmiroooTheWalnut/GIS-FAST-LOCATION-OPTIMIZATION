...
public class FlowControl {
    ...
    public FlowControl(MainFramePanel parent) {
        myParent = parent;
    }
...
//                System.out.println("thread " + i + "finished!");
    ...
    public void correctFacilityLava(int trafficLayerIndex, int numCPUs) {

        int numProcessors = numCPUs;
        if (numProcessors > Runtime.getRuntime().availableProcessors()) {
            numProcessors = Runtime.getRuntime().availableProcessors();
        }

        ParallelRoutingAllWays parallelRoutingAllWays[] = new ParallelRoutingAllWays[numProcessors];

        for (int i = 0; i < numProcessors - 1; i++) {
            parallelRoutingAllWays[i] = new ParallelRoutingAllWays(this, myParent.allData, (int) Math.floor(i * ((myParent.allData.all_Ways.length) / numProcessors)), (int) Math.floor((i + 1) * ((myParent.allData.all_Ways.length) / numProcessors)),trafficLayerIndex);
        }
        parallelRoutingAllWays[numProcessors - 1] = new ParallelRoutingAllWays(this, myParent.allData, (int) Math.floor((numProcessors - 1) * ((myParent.allData.all_Ways.length) / numProcessors)), myParent.allData.all_Ways.length,trafficLayerIndex);

        for (int i = 0; i < numProcessors; i++) {
            //parallelFor[i].myThread.start();
            parallelRoutingAllWays[i].myThread.start();
        }
        for (int i = 0; i < numProcessors; i++) {
            try {
                parallelRoutingAllWays[i].myThread.join();
...
//        System.out.println("myParent.allData.all_Ways.length: " + myParent.allData.all_Ways.length);
...
//                if (myParent.allData.all_Ways[i].myNodes[j].burntBy != null) {
//                    for (int k = 0; k < myParent.allData.all_Ways[i].myNodes[j].burntBy.length; k++) {
//                        if (myParent.allData.all_Ways[i].myNodes[j].burntBy[k] != null) {
//                            Routing routingTest = new Routing(myParent.allData, trafficLayerIndex, k);
//                            routingTest.findPathToFacilityLocation(myParent.allData.all_Ways[i].myNodes[j], myParent.allData.all_Ways[i].myNodes[j].burntBy[k]);
//                            if (routingTest.path == null) {
//                                Routing routingFix = new Routing(myParent.allData, trafficLayerIndex, k);
//                                routingFix.setClosestDifferentFacilityLocation(myParent.allData.all_Ways[i].myNodes[j], myParent.allData.all_Ways[i].myNodes[j].burntBy[k]);
//                            }
//                        }
//                    }
//                }
...
//        System.out.println(((LayerDefinition)myParent.allData.all_Layers.get(passed_ResetLayer)).layerName);
        ...
        for (int i = 0; i < facilityLocations.length; i++) {
            Lava_parallel temp_lava = new Lava_parallel(this, facilityLocations[i].nodeLocation, facilityLocations[i].way, facilityLocations[i].capacity, facilityLocations[i].nodeLocation.lat, facilityLocations[i].nodeLocation.lon, 0, tollOff);
