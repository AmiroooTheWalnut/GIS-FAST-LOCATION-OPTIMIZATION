 ...
 * @version $Revision: 1 $
 */
public class LVQRevised extends AbstractClusterer
    ...
    private void calcStatistics() {
        ...
        for (int i = 0; i < m_clusters.numInstances(); i++) {
            ...
            try {
                ...
                Logger.getLogger(LVQRevised.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    protected Instance denormalizeInstance(Instance inst) {
        ...
        if (m_normalizeAttributes) {
            for (int noa = 0; noa < inst.numAttributes(); noa++) {
                inst.setValue(noa, (inst.value(noa)*inst.weight() * (m_attributeMax[noa] - m_attributeMin[noa]) + (m_attributeMax[noa] + m_attributeMin[noa])) / 2);
            }
        }
        return inst;
    }
    ...
    protected Instance normalizeInstance(Instance inst) {
        ...
        for (int noa = 0; noa < m_instances.numAttributes(); noa++) {
            if (inst.value(noa)*inst.weight() > m_attributeMax[noa]) {
                max = inst.value(noa)*inst.weight();
            } else {
                max = m_attributeMax[noa];
            }
            if (inst.value(noa)*inst.weight() < m_attributeMin[noa]) {
                min = inst.value(noa)*inst.weight();
            } else {
                min = m_attributeMin[noa];
            }
            if ((max - min) != 0) {
                inst.setValue(noa, -1 + 2 * (inst.value(noa)*inst.weight() - min) / (max - min));
            } else {
                inst.setValue(noa, inst.value(noa)*inst.weight());
            }
        }
        return inst;
    }
            ...
            for (int noa = 0; noa < inst.numAttributes(); noa++) {
                ...
                for (int i = 0; i < inst.numInstances(); i++) {
                    if (!inst.instance(i).isMissing(noa)) {
                        value = inst.instance(i).value(noa)*inst.instance(i).weight();
                        if (value < min) {
                            min = value;
                        }
                        if (value > max) {
