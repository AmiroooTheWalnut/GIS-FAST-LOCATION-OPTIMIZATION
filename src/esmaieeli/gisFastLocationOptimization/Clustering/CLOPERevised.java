...
public class CLOPERevised
  ...
  private class CLOPECluster implements Serializable {
	...
	if (N == 0) {
	  deltaprofit = ((double)(S_new*inst.weight()) / Math.pow(W_new, r));
	} else {
	  profit = ((double)(S * N * inst.weight()) / Math.pow(W, r));
	  profit_new = ((double)(S_new * (N + 1) *inst.weight()) / Math.pow(W_new, r));
  ...
  public int AddInstanceToBestCluster(Instance inst) {
	...
	for (int i = 0; i < inst.numAttributes(); i++) {
      ...
      deltamax = ((double)(tempS*inst.weight()) / Math.pow(tempW, m_Repulsion));
  ...
  public static void main(String[] argv) {
    runClusterer(new CLOPERevised(), argv);
  }
}
